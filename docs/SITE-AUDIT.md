...
### MEASUREMENT BOUNDARY: brand copy, CFB framing, four-league app copy [MANUAL, added 2026-08-25]

- 2026-08-25 moves copy on every URL (chrome), FAQPage structured data on all 169 team pages and the homepage, and meta descriptions on the root, /teams, /follow, six promo collections, /about, /cfb and /playoffs. Any sitewide, team-page or FAQ-rich-result trend crossing this date compares different text. Merge 02e6fa3. Rules recorded in section 9.
- WHAT SHIPPED. The tagline "Find the game, plan the night." replaced "Every promo at every game" (eyebrow on the served homepage, h1 on the gate-off one, Organization.slogan, one shared og:image alt replacing five literals in two spellings). The college corpus is named as a separate sentence on the homepage WebSite and Organization descriptions, the team-count FAQ and both hero ledes; /cfb stopped promising theme nights. The nav spells out "College football", both footers link /cfb, the /venues up-link stops saying "All CFB promos", and the aria-label "CFB promotional schedule" that shipped twice on every production page is gone. Nine app-coverage claims now name the four leagues the app covers; on the 47 WNBA and NFL team pages the app pitch became a weekly-email card and FAQ answers 5 and 5e point at the email. Every hand-typed 169 and six-league list derives from one source. /api/og was removed (orphan card generator, zero consumers, zero backlinks). /playoffs head corrected from MLB to NBA. /about meta description reworded and shortened to 156 characters.
- VISIBLE WORD DELTA by page type, measured prod vs preview on cache-busted served HTML with the same extractor as the 2026-08-21 boundary. Homepage +49 (college clause in the lede, FAQ answers 1 and 3 rewritten, plus the footer link every page gets). Team page, app league (MLB, NBA, NHL, MLS) -9: FAQ 5 lost the "Download it on iOS or Android for a free calendar view" sentence and gained "free on this site". Team page, WNBA and NFL +9 to +12 (Vikings +9, Cowboys +9, Aces +12, Packers +12; the Lynx and Liberty reads carried expired-promo drift and are not copy deltas): the app pitch card (about 40 words) left and the weekly-email card (about 70) arrived, FAQ 5e shrank by 6. /promos/this-week +9 (FAQ 3). /about -3 visible (two retired every-X sentences shortened; the meta description, not visible, went from 174 to 156 characters). /cfb -1 ("theme nights" out, "schedules" in). llms.txt +6, manifest.json +6. /teams, /follow, /my-teams, /download, /venues, /playoffs, /best-promos, /team-rankings, venue hubs, CFB school and rivalry pages: 0 beyond the chrome (league-order consolidation and alt text change words but not counts; the footer link adds 2 visible words everywhere).
- STRUCTURED DATA: homepage Organization gains slogan and a second description sentence; WebSite description gains the college clause; FAQPage answers 1 to 3 rewritten on the homepage; team-page FAQPage answers 5 and 5e rewritten on all 169 pages (47 of them to the email version); /promos/this-week FAQ 3 rewritten; /about dateModified 2026-08-25.
- NOT CHANGED, deliberately: the homepage founder block league order (by team count) and the /venues description order (by coverage weight) each have a stated rationale and stay. YEAR = getFullYear() in four promo collections' metadata is a standing house-rule violation outside this pass.
...
- **Any tool that loads a production page programmatically MUST set a real User-Agent and use a fresh browser context per load.** Raptive's `ads.min.js` does not initialise for a UA announcing `HeadlessChrome`, which is Puppeteer's and headless Chrome's default. The page serves normally, ads simply never start, so a tool that does not set a UA reports ZERO ADS ON A HEALTHY PAGE and looks like a site defect rather than an instrument defect. Symptom, in this order: `window.adthrive.config.dynamicAds` is absent, and `document.body` carries no `adthrive-device-*` class. This has now cost two separate tools: the CWV capture harness (`scripts/capture-cwv-baseline.py`, whose header records the desktop-UA correction) and the content-placement checker (`promo-pipeline/scripts/check-content-placement.js`, whose first run returned "dynamicAds absent" on all three desktop rows while those pages were serving ads in a headed browser). Derive the UA from the browser's own (`browser.userAgent()` with `HeadlessChrome` replaced) rather than pinning a version string that will rot. The companion guard is storage: Raptive persists `adthrive_user_session` and `adthrive_abgroup` in localStorage PER ORIGIN and pins device classification from them, so a prior phone load makes a later desktop load report `device: phone` on the same origin. A fresh browser context per load is cleaner than clearing keys because it also drops cookies. Third symptom worth knowing while you are here: their init is async, so a fixed sleep races it and intermittently reports the same "no `adthrive-device-*` class" on a healthy page; poll for readiness instead, and let "inconclusive" mean Raptive never initialised rather than that you looked too early.
...
## 9. Copy rules [MANUAL, added 2026-08-25]

Three rules a future thread will get wrong without a written line. Each has a single source in code; edit the source, never the rendered string.

- **THE BRAND LINE is "Find the game, plan the night." with the period.** One constant, `BRAND_TAGLINE` in `src/lib/brand.ts`, renders it as the redesign hero eyebrow, the gate-off homepage h1, `Organization.slogan` in the homepage schema, and the shared social-card alt (`OG_IMAGE_ALT` in `src/lib/og.ts`). It is a tagline, not a description: it does not go into meta descriptions, schema descriptions, footer coverage paragraphs, or app blurbs, and it does not replace the h1 on the served variant ("Find the games worth going to."). The retired line and its family ("Every promo at every game", "Every giveaway, every team", "Every promo. Every team.", "Every promo, every team, every league") are gone from code; `public/og-image.png` still carries the old one until it is regenerated (known-issues entry 38).
- **THE CFB FRAMING RULE: 169 teams across six leagues, plus 87 college football programs. Never "seven leagues".** The pro teams live in `teams` and span MLB, NBA, NFL, NHL, MLS and WNBA. The 87 schools live in `cfbSchools`, carry schedules, venues and rivalries, and have no promo data and no scores. `src/lib/coverage-counts.ts` is the one derivation: `teamCount` and `leagueCount` are pro-only by construction and `cfbSchoolCount` rides beside them, so no sentence can add them together. The clause belongs on surfaces that describe what the site covers (homepage schema descriptions, the homepage FAQ, the hero lede, /about, llms.txt). It must not be added to anything describing promos, promo counts, cadence, starring or rankings. The reverse error is the one that actually shipped: promo vocabulary on the college corpus ("CFB promotional schedule" as an aria-label, "All CFB promos", "theme nights" on /cfb). Reference wording is `aboutLede` in `src/lib/about-copy.ts`. Counts are never typed: `getCoverageCounts()` in server code, and `cfbSchoolCount` comes from `getAllCfbSchoolIds`, which reads the collection (87 today; the code comments that said 86 were stale and are fixed).
- **THE FOUR-LEAGUE APP RULE: the app covers MLB, NBA, NHL and MLS.** `APP_LEAGUES` in `src/lib/coverage-counts.ts` is the source and `appLeagueList` renders it. Any sentence that attributes coverage to the app names those four; six-league and all-169 claims belong to the website. On WNBA and NFL team pages the app pitch renders the weekly-email card instead (`AppPushPitch` with `league`), the FAQ answers 5 and 5e point at the weekly email, and the "How do I find promos" plug says the app does not carry the league yet. The weekly email is the Tuesday digest (`vercel.json` cron `0 17 * * 2`); it is weekly, never a promo-day reminder, which remains the Pro app's claim on app-league pages only. League lists render in `LEAGUE_ORDER`; the two surfaces that order differently on purpose are the homepage founder block (by team count, the league-agnostic rule) and the /venues index and description (by coverage weight).

### 9a. A COPY OR CLAIM AUDIT IS VERIFIED AGAINST SERVED HTML, NEVER AGAINST A SOURCE GREP [added 2026-08-29]

**A source grep answers "where is this string written". A claim audit needs "where does this string ship". They are different questions, and shared layout and component files are where they diverge.** One line in a shared component reaches every page that mounts it, so a page-level grep over `src/app/promos/bobbleheads/page.tsx` reads clean while the claim is on that page and three others. Grep to FIND candidates. Verify by fetching the rendered page.

**Fetch it with a cache-busting curl and check all five layers**, because a claim removed from the body can survive in a layer nobody looked at: `<title>`, `<meta name="description">`, the `og:` tags, the JSON-LD blocks, and the visible body. A pass on four of five is not a pass.

Three instances from the 2026-08-29 pre-filing sweep, each of which a source grep declared clean:

1. **`llms.txt` restated a claim that had just been deleted from the page it was found on.** The eBay resale sentence was removed from `/promos/bobbleheads` because no resale observation is stored anywhere in the system. `llms.txt` independently carried "with current eBay resale availability", and alongside it "Complete lists of upcoming promotional events for each of 169 teams" — a flatter completeness claim than the sentence directly above it, which had already been corrected in the same file in the same pass. Fixing a claim on the page it was reported on does not fix the machine-readable file that repeats it, and that file is what AI crawlers read first.
2. **`src/lib/promo-helpers.ts` team-FAQ answers survived a first pass.** Two answers were rewritten; two more in the same function were not, and those ship inside FAQPage structured data on 169 pages. The file had already been edited, so it looked done.
