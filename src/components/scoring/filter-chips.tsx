...
type FilterChipsViewProps<T extends string> = {
  options: readonly FilterChipOption<T>[];
  current: T;
  // Fires on every chip tap, including a tap on the already-active chip;
  // callers that care must guard on `next === current` themselves.
...
// Presentational chip row with no hooks. Exists so controlled callers
// (best-promos) can render chips without mounting useSearchParams, which
// on a statically rendered route would bail the whole subtree out of the
// prerendered HTML.
export function FilterChipsView<T extends string>({
  options,
  current,
  onSelect,
  ariaLabel,
  variant = 'dark',
}: FilterChipsViewProps<T>) {
  return (
...
// URL-synced chips. Reads useSearchParams, so on a statically rendered
// route the mounting subtree bails to client rendering up to the nearest
// Suspense boundary; pages whose crawlable content shares that boundary
// should use FilterChipsView in controlled mode instead (see
// best-promos-browser.tsx).
