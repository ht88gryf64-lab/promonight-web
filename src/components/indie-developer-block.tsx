...
export function IndieDeveloperBlock({
  ...
  variant?: 'dark' | 'light';
  /** Derived from getCoverageCounts() by the page. Required so the count and
   *  the league list in this prose cannot go stale independently of the rest
   *  of the homepage (the list was a hand-typed literal in its own order). */
  teamCount: number;
  leagueList: string;
}) {
    ...
    <section className={`py-20 px-6 border-t ${light ? 'border-rd-line' : 'border-border-subtle'}`}>
      <div className="max-w-3xl mx-auto">
        ...
        <div className={light ? 'space-y-4 text-rd-ink-soft text-[15px] leading-relaxed' : 'space-y-4 text-text-secondary text-[15px] leading-relaxed'}>
          ...
          <p>
            So I spent a few months building it. PromoNight started with just the Twins and now tracks every giveaway, theme night, food deal, and kids event across all {teamCount} teams in {leagueList}. If you&apos;ve ever shown up to a game and found out you missed bobblehead night by 24 hours, this is for you.
          </p>
        </div>
