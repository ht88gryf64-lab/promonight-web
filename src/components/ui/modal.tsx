...
interface ModalProps {
  ...
  ariaLabel?: string;
  /** Surface theme. 'dark' (default) preserves the original team-page modal;
  ...
  return (
    <dialog
      ...
      className={`
        ...
        sm:max-h-[85vh]
        ${light ? 'bg-rd-cream text-rd-ink sm:border-rd-line' : 'bg-bg-card text-white sm:border-border-subtle'}
    ...
    >
      <div className="relative w-full h-full sm:h-auto sm:max-h-[85vh] overflow-y-auto">
        <button
          ...
          aria-label="Close"
          onClick={onClose}
          className={`sticky top-3 float-right mr-3 z-10 w-9 h-9 flex items-center justify-center rounded-full transition-colors ${light ? 'bg-rd-ink/[0.06] hover:bg-rd-ink/[0.12] text-rd-ink' : 'bg-white/10 hover:bg-white/20 text-white'}`}
        >
